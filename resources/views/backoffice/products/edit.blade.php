                    ...
                    <div class="card-body">
                        <div class="form-check form-switch mb-2">
                            <input class="form-check-input" type="checkbox" name="is_published" id="isPublished"
                                value="1" @checked(old('is_published', $product->is_published))>
                        ...
                        <div class="form-check form-switch">
                            <input class="form-check-input" type="checkbox" name="is_occasion" id="isOccasion"
                                value="1" @checked(old('is_occasion', $product->is_occasion))>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <select name="tags[]" class="form-select" multiple id="tags-select">
                            @foreach ($tags as $tag)
                                <option value="{{ $tag->id }}" @selected(collect(old('tags', $product->tags->pluck('id')))->contains($tag->id))>
                                    {{ $tag->name }}
                                </option>
                            @endforeach
                        </select>
...
@section('scripts')
    ...
    <script>
        ClassicEditor
            ...
            .then(editor => {
                editor.ui.view.editable.element.style.minHeight = '350px'; // ajuste la hauteur
            })
            .catch(error => {
                console.error(error);
            });
